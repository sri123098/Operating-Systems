...
static int test_block(char *scratchpad, int bytes_read)
...
sg_init_one(&sg, scratchpad, bytes_read);
// struct scatterlist dest;
//sg_init_one(&dest,destpad , bytes_read);
// bytes_read is the number of the bytes to process from src
...
skcipher_request_set_crypt(req, &sg, &sg, bytes_read, iv_store);
...
//        int t=test_block(); 
	...
	while(read_flag)
	{
		...
		int t = test_block(data,bytes_read);
		printk("After encryption and then decryption\ndata %s and bytes_read %ld", data, bytes_read);
	//	goto out;
