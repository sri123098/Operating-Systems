...
asmlinkage long cpenc(void *arg)
{
           ...
           if((k_arg->arg1 == NULL) || (k_arg->arg2 == NULL) || (k_arg->arg3 == NULL) || (k_arg->flag == 0)) {
           ...
           /*extra check for missing arguments*/
            ...
            if((!k_arg->arg1) || (!k_arg->arg2) || (!k_arg->arg3)){
                    printk("Missing arguments\n");
                    retval = -EINVAL;
                    goto out;
            }
            printk("flag %d",flag);
            printk("input %s", k_arg->arg1);
            printk("output %s", k_arg->arg2);
           ...
           printk("\nUSER key\n");
            ...
            if (!fd_in || IS_ERR(fd_in)) {
                printk("%ld\n", PTR_ERR(fd_in));
                retval=-ENOENT;//No such file or device
                goto out;
            }
...
//            if(!(fd_out->f_op->write)) {
//                  printk("Write permission error \n");
//                  retval = -EPERM;
//                  goto out;
//            }
             /*Checking whether output is a regular file?*/
            if ((flag ==4) && (!S_ISREG(fd_out->f_path.dentry->d_inode->i_mode))){
                printk("Input is not a regular file");
                retval = -EBADF;
                goto out;
            }
            ...
            /*
             *Way you need to order them is to structure them with different out statements.
             *First defined last free.
             *The idea is to free kmalloc 
             *cipher block
             *filp_open block
             *
             */
            out:
                putname(file_in);
                putname(file_out);
			    if ((!IS_ERR(fd_out)) && (retval != -EINVAL) && (retval!=0)){
			        filp_close(fd_out,0);
