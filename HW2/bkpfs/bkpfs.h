...
{
	printk("setting the lower inode");
	//dump_stack();
...
{
	printk("setting the bkpfs_i->lower_inode=val");
...
{
	printk("pathcpy %s", src->dentry->d_iname);
					 ...
					 struct path *lower_path)
{
	...
	printk("bkpfs_set_lower_path %s", lower_path->dentry->d_iname);
	spin_unlock(&BKPFS_D(dent)->lock);
	return;
}
static inline void bkpfs_reset_lower_path(const struct dentry *dent)
{
	...
	printk("bkpfs_reset_lower_path %s", dent->d_iname);
	spin_unlock(&BKPFS_D(dent)->lock);
	return;
}
static inline void bkpfs_put_reset_lower_path(const struct dentry *dent)
{
	...
	BKPFS_D(dent)->lower_path.dentry = NULL;
	BKPFS_D(dent)->lower_path.mnt = NULL;
	printk("bkpfs_put_reset_lower_path %s", dent->d_iname);
	spin_unlock(&BKPFS_D(dent)->lock);
	path_put(&lower_path);
	return;
}
...
static inline struct dentry *lock_parent(struct dentry *dentry)
{
	...
	printk("lock_parent %s", dentry->d_iname);
