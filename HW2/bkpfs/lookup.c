				      ...
				      struct path *lower_parent_path)
{
	...
	printk("instantiate a new negative dentry");
	this.name = name;
	this.len = strlen(name);
	this.hash = full_name_hash(lower_dir_dentry, this.name, this.len);
	/*this.hash is the integer */
	/* d_lookup if the dentry */	
	lower_dentry = d_lookup(lower_dir_dentry, &this);
	if (lower_dentry->d_inode ==NULL)
		printk("Null Inode doesnot exist");
	else
		printk("Inode exist\n");
	if (lower_dentry)
		goto setup_lower;
			     ...
			     unsigned int flags)
{
	...
	if(strncmp(dentry->d_name.name, ".bkp.",5 )==0){
		ret = ERR_PTR(-ENOENT);
		goto out;
	}
