...
rm -rf del b
ls -a /test/HW2_v6/ | grep ".bkp.a" | tail -n4 > del

/usr/src/hw2-skalluri/CSE-506/bkpctl -d oldest /mnt/bkpfs/a > del2

/usr/src/hw2-skalluri/CSE-506/bkpctl -l /mnt/bkpfs/a | grep ".bkp.a" > b

if cmp del b;then
    echo "******************************"
    echo "IOCTL Test case for Deleting the oldest backup ---------> passed"
    echo "******************************"
else
    echo "******************************"
    echo "IOCTL Test case for Deleting the oldest backup ---------> failed"
    echo "******************************"
fi
