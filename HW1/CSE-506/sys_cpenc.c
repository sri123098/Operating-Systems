...
int copy_fs(struct file *fd_in, struct file *fd_out, char *data, struct kstat *stat)
{
		...
		/*To ensure the size of the file matches */
...
*@ivdata:     Initialization vector used in cipher handle(inside the function)
*@ivlen:      IV len which can be passed as a user parameter
*/

int encrypt_fs(struct file *fd_in, struct file *fd_out, char *data, struct kstat *stat, struct skcipher_request *req, struct scatterlist sg, char *hash_key, unsigned int ivlen)
{
		ssize_t bytes_read, bytes_write;
		bool read_flag = true;
		int retval = 0;
		...
		while (read_flag) {
			...
			bytes_write = vfs_write(fd_out, ivdata, ivlen, &fd_out->f_pos);
			if (bytes_write < 0) {
				retval = -EINTR;
				goto out;
			}	    
		...
		/*To ensure the size of the file matches 16 is for hashed password
 		* size*ivlen is for the ivdata bytes in the input file */
		if (fd_out->f_pos != (stat->size + 16 + size*ivlen)) {
			printk(KERN_ERR "issue in writing in copy");
			retval = -EINTR;
		}
...
*@ivlen:      ivdata len used to retrieve the ivdata from the encrypted file
*/

int decrypt_fs(struct file *fd_in, struct file *fd_out, char *data, struct kstat *stat, struct skcipher_request *req, struct scatterlist sg, char *hash_key, unsigned int ivlen)
{
		ssize_t bytes_read, bytes_write;
		bool read_flag = true;
		int retval = 0;
		...
		while (read_flag) {
			size+=1;
			bytes_read = vfs_read(fd_in, data, ivlen, &fd_in->f_pos);
			if (bytes_read < 0) {
				retval = -EINTR;
				goto out;
			}
			memcpy(ivdata, data, ivlen);
			bytes_read = vfs_read(fd_in, data, BUF_SIZE, &fd_in->f_pos);
			if (bytes_read < 0) {
				retval = -EINTR;
				goto out;
...
asmlinkage long cpenc(void *arg)
{
		...
		*/
		...
		if (flag & 0x1) {
		    retval = encrypt_fs(fd_in, fd_out, data, stat, req, sg, hash_key, 16);
		    if (retval != 0)
			printk("error during encryption\n");
		} else if (flag & 0x2) {
		    retval = decrypt_fs(fd_in, fd_out, data, stat, req, sg, hash_key, 16);
		    if (retval != 0)
