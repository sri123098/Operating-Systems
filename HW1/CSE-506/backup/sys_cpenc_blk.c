...
static int test_block(void *input_data,void *output_data, void *key, int vfs_read_output)
{
   struct crypto_blkcipher *blkcipher;
   blkcipher = crypto_alloc_blkcipher("ctr-aes-aesni", 0, 0);
    if (IS_ERR(blkcipher)) {
        pr_info("could not allocate blkcipher handle\n");
        return PTR_ERR(blkcipher);
    }
struct scatterlist *sg_in, *sg_out;
sg_in= kmalloc(sizeof(struct scatterlist),GFP_KERNEL);
sg_out= kmalloc(sizeof(struct scatterlist),GFP_KERNEL);
//the caller provided key is set for the single block cipher 
////return 0 on success and -1 on failure
////32 has been changed from skcipher to block cipher
////const *u8 key; keylen is the third parameter
...
desc->tfm=blkcipher;
desc->flags=0;
desc=kmalloc(sizeof(blkcipher_desc),GFP_KERNEL);
                 sg_set_buf(sg_in, const (void *)input_data, vfs_read_output);
                 sg_set_buf(sg_out, const (void *)output_data, vfs_read_output);
        //How to set the desc?          
	int check=crypto_blkcipher_encrypt(desc,sg_out, sg_in, vfs_read_output);
   if (check)
        goto out;
